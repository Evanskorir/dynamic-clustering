...
class Pca:
    ...
    def PCA_apply(self, n_components=2):
        ...
        for company, scaled_data in self.data_scaled.items():
            # Apply PCA to each time point (42 time points with 4 variables)
            pca_result = pca.fit_transform(scaled_data)  # Apply PCA to each company's data (42, 4)
            company_results.append(pca_result)  # Resulting shape for each company: (42, reduced dim)

        # Convert the list of arrays into a 3D numpy array: (company's size, 42, reduced dim)
